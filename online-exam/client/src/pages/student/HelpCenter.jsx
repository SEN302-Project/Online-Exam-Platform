...
import {
  Home, CalendarClock, ScanFace, MonitorCheck, Radio, TrendingUp, BookOpen, HelpCircle, Sparkles, ShieldCheck,
  Search, ChevronDown, MessageSquare, Mail, Phone, FileText, Camera, Wifi, Shield,
} from "lucide-react";
...
const faqs = [
  { q: "What do I need to take an online exam?", a: "You'll need a computer with a working webcam and microphone, a stable internet connection (at least 2 Mbps), and a supported browser (Chrome, Firefox, Edge, or Safari). You should also be in a quiet, well-lit room." },
  { q: "Can I use my phone to take an exam?", a: "Mobile devices are currently not supported for full examinations due to proctoring requirements. You can review study materials and check your schedule on mobile, but exams must be taken on a desktop or laptop." },
  { q: "What happens if my internet disconnects during an exam?", a: "Your answers are auto-saved every 30 seconds. If you disconnect, you can reconnect within 5 minutes and continue from where you left off. The proctor will be notified and may verify your identity again." },
  { q: "How is plagiarism detected?", a: "We use a combination of text similarity matching (against your peers and online sources) and AI-generated content detection. All submissions are scanned automatically." },
  { q: "Can I flag a question and come back to it later?", a: "Yes! Click the flag icon next to any question. Flagged questions are highlighted in the navigation panel so you can easily return to them before submitting." },
  { q: "What if my webcam isn't working?", a: "Run the System Check from your dashboard. If it fails, check that your browser has camera permission, no other app is using the camera, and try restarting your browser." },
  { q: "Can I switch tabs or open other apps during an exam?", a: "No. Switching tabs, minimizing the browser, or opening other apps is flagged as a violation. Repeated violations may result in your exam being voided." },
  { q: "How long does it take to get my results?", a: "MCQ and True/False questions are graded instantly. Essay and coding questions require instructor review, which typically takes 3-5 business days." },
];
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-10 py-6 sm:py-8 max-w-full lg:max-w-[1400px]">
          <div className="card overflow-hidden mb-8">
            <div className="bg-gradient-to-br from-ink-900 to-ink-950 text-white p-6 sm:p-10 relative">
              ...
              <div className="relative max-w-2xl">
                ...
                <p className="mt-2 text-ink-300">Find answers to common questions or reach out to our support team.</p>
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {categories.map((cat) => {
                const colors = { accent: "bg-accent-50 text-accent-600", success: "bg-success/10 text-success", warning: "bg-warning/10 text-warning", ink: "bg-ink-100 text-ink-700" };
                return (
          ...
          <section className="mb-8">
            ...
            <div className="card overflow-hidden">
              ...
              {filtered.length === 0 && <div className="p-12 text-center text-ink-400">No results found. Try different keywords.</div>}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              {/* Live Chat - placeholder, needs backend */}
              <button className="card p-5 text-left hover:shadow-lg transition group">
                ...
                <div className="mt-4 font-semibold text-ink-900 group-hover:text-accent-600 transition">Live Chat</div>
                <div className="text-sm text-ink-500 mt-0.5">Avg response: 2 min</div>
              </button>

              {/* Email - opens mail client */}
              <a href="mailto:support@proctera.com" className="card p-5 text-left hover:shadow-lg transition group block">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-success/10 text-success">
                  <Mail size={18} />
                </div>
                <div className="mt-4 font-semibold text-ink-900 group-hover:text-accent-600 transition">Email</div>
                <div className="text-sm text-ink-500 mt-0.5">support@proctera.com</div>
              </a>

              {/* Phone - opens phone app */}
              <a href="tel:+18005550100" className="card p-5 text-left hover:shadow-lg transition group block">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-warning/10 text-warning">
                  <Phone size={18} />
                </div>
                <div className="mt-4 font-semibold text-ink-900 group-hover:text-accent-600 transition">Phone</div>
                <div className="text-sm text-ink-500 mt-0.5">+1 (800) 555-0100</div>
              </a>
            </div>
          </section>
        </main>
