...
import {
  LogOut, User, Bell, GraduationCap, Search, FileText, AlertTriangle, CheckCircle2,
  Sun, Moon, Menu,
} from "lucide-react";
...
export default function Navbar({ minimal = false, showSearch = false, fullWidth = false, onMenuClick }) {
  ...
  const containerClass = fullWidth
    ? "flex h-16 items-center justify-between gap-2 sm:gap-3 px-3 sm:px-6"
    : "mx-auto flex h-16 max-w-7xl items-center justify-between gap-2 sm:gap-3 px-3 sm:px-6";
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-ink-100 bg-white/80 backdrop-blur-md">
      <div className={containerClass}>
        ...
        {onMenuClick && (
          <button onClick={onMenuClick} className="btn-ghost lg:hidden !p-2 shrink-0" title="Menu">
            <Menu size={20} />
          </button>
        )}
        ...
        {!minimal && user && (
          <div className="flex items-center gap-1 shrink-0">
            ...
            <div className="relative" ref={notifRef}>
              ...
              {notifOpen && (
                <div className="absolute right-0 top-full mt-2 w-[calc(100vw-1.5rem)] sm:w-80 card overflow-hidden animate-slide-up z-50">
            ...
            <Link to="/profile" className="flex items-center gap-2 rounded-lg p-1 sm:px-2 sm:py-1.5 hover:bg-ink-50 transition">
